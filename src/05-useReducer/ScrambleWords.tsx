...
export const ScrambleWords = () => {
    ...
    const [currentWord, setCurrentWord] = useState(words[0]); //palabra correcta
    const [scrambledWord, setScrambledWord] = useState(scrambleWord(currentWord)); //palabra a presentar
    const [guess, setGuess] = useState(''); //lo que escribe el usuario
    ...
    console.log("lista palabras", words);
    ...
    const handleGuessSubmit = (e: React.FormEvent) => {
        ...
        if (guess === currentWord) {
            ///remover de la lista de palabras la palabra acertada
            console.log(words);
            const nuevaPalabra = words[1];
            setPoints(points + 1);
            setCurrentWord(nuevaPalabra);
            setScrambledWord(scrambleWord(nuevaPalabra));
            setGuess('');
            const nuevaLista = words.filter((palabra) => palabra != currentWord)
            setWords(nuevaLista);
             console.log("acerto en la palabra ", currentWord);

        } else {
            const numeroErrores = errorCounter + 1;
            setErrorCounter(numeroErrores);
            if (numeroErrores === maxAllowErrors) {
                setIsGameOver(!isGameOver);
            }
            setGuess('');
            console.log("Se equivoco en la palabra ", currentWord);
        }
    console.log( "Numero de palabras faltantes : ", words.length)
    };

    const handleSkip = () => {
        const nuevaPalabra = words[1];
        const palabraAnterior = currentWord;
        setCurrentWord(nuevaPalabra);
        setScrambledWord(scrambleWord(nuevaPalabra));
        setGuess('');
        setSkipCounter(() => skipCounter + 1)
        console.log("palabra eliminada de la lista", currentWord);
        const nuevoArrayPalabras = words.filter((palabra) => palabra !== palabraAnterior)
        setWords(nuevoArrayPalabras);
        console.log("lista resultante", nuevoArrayPalabras);
