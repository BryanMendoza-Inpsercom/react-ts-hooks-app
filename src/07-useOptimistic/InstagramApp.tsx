...
export const InstagromApp = () => {
  ...
  const [optimisticComments, addOptimisticComment] = useOptimistic(
    comments,
    (currentComments, newCommnet: string) => {
      lastId++;
      return [
        ...currentComments,
        {
  ...
  const handleAddComment = async (formData: FormData) => {
    const menssageText = formData.get("post-message") as string;

    addOptimisticComment(menssageText);

    startTransition(async () => {
      console.log("Nuevo comentario", menssageText);
